...
Do not confuse the *Rules Short Hand Syntax* with the [Healix Expression Grammar](/docs/dynamic-data-model/healix-calculation-grammar). The rules syntax is further simplified and always assumes a comparison will be provided.  The  [Healix Expression Grammar](/docs/dynamic-data-model/healix-calculation-grammar) can compute an arbitray calculation and is not necessarily a comparison.
...
Date comparisons are yet to be implemented
